...
== --NATURAL JOIN을 하면 쿼리문이 훨씬 줄여질수 있다. 
...
A.PRODUCT_NAME, B.CATEGORY_NAME
	...
	INNER JOIN
		CATEGORIES B 
	ON A.CATEGORY_OD =B.CATOGORY_ID;
==
SELECT 
A.CATEGORY_ID, A.PRODUCT_ID,
A.PRODUCT_NAME, B.CATEGORY_NAME
	FROM PRODUCTS A, CATEGORIES B
where A.CATEGORY_OD = B.CATOGORY_ID;
...
select 	*
	from city a
natural join country b; --> 아무것도 나오지 않음 why? last_update값이 동일하지 않아서, 

-- 이런 경우에는 natural join 시에는 last_update 컬럼까지 inner 조인에 성공해야만 값이 나옴 
--natural 조인을 잘 ㅅ사용 하지 않는다. -> 예상치 못하게 데이터가 나오지 않을 수 있다. 
SELECT 	*
	FROM CITY A
